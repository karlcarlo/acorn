define(['lib/ajax-upload/ajaxupload'], function (AjaxUpload) {
    ...
    var createDialog = function () {
        ...
        if (isImage) {
          ...
          var img_uploader = new AjaxUpload(upload_img_btn, {
            ...
            data: {
              description: 'auto uploaded image'
            },
            onComplete: function (file, response) {
              if (response.success) {
                input.value = response.file.url;
              }
              else {
                alert("上传失败，请重试。");
              }
  ...
  function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions) {
    ...
    function makeSpritedButtonRow() {
      ...
      var redoTitle = /win/.test(nav.platform.toLowerCase()) ? "重做 - Ctrl+Y" : "重做 - Ctrl+Shift+Z"; // mac and other non-Windows platforms
  ...
  commandProto.doLinkOrImage = function (chunk, postProcessing, isImage) {
    ...
    if (chunk.endTag.length > 1 && chunk.startTag.length > 0) {
      ...
      if (isImage) {
        if (!this.hooks.insertImageDialog(linkEnteredCallback)) ui.prompt(imageDialogText, imageDefaultText, linkEnteredCallback, isImage);
      }
      else {
